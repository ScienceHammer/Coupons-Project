...
public class CompanyService extends ClientService {
	 ...
	 * 
	 */
	public Coupon addCoupon(Coupon coupon, MultipartFile multiPartImage) throws CouponsSystemException {
		...
		if (multiPartImage != null) {
			String imagePath = storageService.storeFile(multiPartImage, "coupon_" + coupon.getId() + ".jpeg");
	 ...
	 * 
	 */
	public Coupon updateCoupon(Coupon coupon, MultipartFile multiPartImage) throws CouponsSystemException {
		if (coupon == null) {
		...
		if (multiPartImage != null) {
			String imagePath = storageService.storeFile(multiPartImage, "coupon_" + couponDb.getId() + ".jpeg");
			couponDb.setImage(imagePath);
		}
		return couponDb;
	}
	...
	public Coupon deleteCoupon(int couponID) throws CouponsSystemException {
		...
		if (optional.isPresent()) {
			...
			storageService.deleteFile("coupon_" + couponID + ".jpeg");
			return optional.get();
		} else {
	...
	public List<Coupon> getCompanyCoupons(Category category, Double maxPrice) {
		return couponRepositories.findAllByCompanyIdAndCategoryAndPriceLessThanEqual(this.companyID, category,
				maxPrice);
	}
