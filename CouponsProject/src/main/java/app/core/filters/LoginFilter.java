				...
				case COMPANY:
					if (url.contains("/company")) {
						...
						if (tokenUtil.validateToken(jwtToken, company.getEmail())) {
							if (company.getPasswordTimeStamp() == null || tokenUtil.extractIssued(jwtToken)
									.after(new Date(company.getPasswordTimeStamp()))) {

								ClientService clientService = loginManager.login(clientEmail, company.getPassword(),
										ClientType.COMPANY);
								req.setAttribute("service", clientService);
								chain.doFilter(request, response);
				...
				case CUSTOMER:
					if (url.contains("/customer")) {
						...
						if (tokenUtil.validateToken(jwtToken, customer.getEmail())) {
							if (customer.getPasswordTimeStamp() == null || tokenUtil.extractIssued(jwtToken)
									.after(new Date(customer.getPasswordTimeStamp()))) {
								
								ClientService clientService = loginManager.login(clientEmail, customer.getPassword(),
										ClientType.CUSTOMER);
								req.setAttribute("service", clientService);
								chain.doFilter(request, response);
